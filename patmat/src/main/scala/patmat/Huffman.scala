...
object Huffman {
   ...
   */
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table.find(_._1.equals(char)) match {
  ...
  def mergeCodeTables(left: CodeTable, right: CodeTable): CodeTable = {
    def transform(x: Int, p: (Char, List[Bit])): (Char, List[Bit]) = p match {
      case (c, bs) => (c, x :: bs)
    }
    left.map(transform(0, _)) ::: right.map(transform(1, _))
  }
